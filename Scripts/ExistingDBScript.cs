...
public class ExistingDBScript : MonoBehaviour {
	...
	void Start () {
		...
		ButtonReset.onClick.AddListener(OnBtnResetDB);
		...
		if (words.Count() <= 0)
		{
			...
			uiSentence.text = "您可以重置数据库，要未学习状态。";
		}
		else
		{
	...
	public void OnBtnResetDB()
	{
		ds._connection.DropTable<words>();
		ds._connection.Execute("CREATE TABLE words AS SELECT * FROM basewords;", new object[] { });
		BtnReset.SetActive(false);
		btnCheck.GameObject().SetActive(true);
		BtnNext.GameObject().SetActive(true);
	...
	{   
		...
		if (selAnswer == explain)
		{
			...
			string sqlcommand = "DELETE FROM words WHERE word='" +
			                    word +
			                    "' AND explain='" +
			                    explain +
			                    "' AND example='" +
		...
		else
		{
			...
			string sqlcommand = "INSERT INTO problemwords (word, explain, example, source) VALUES ('" +
			                    word +
			                    "','" +
			                    explain +
			                    "','" +
